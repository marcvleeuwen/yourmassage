document.addEventListener('DOMContentLoaded', function () {
    ...
    const sections = document.querySelectorAll(".session-page");
    const menu_links = document.querySelectorAll("aside section");

    // functions to add and remove the active class from links as appropriate
    const makeActive = link => menu_links[link] && menu_links[link].classList.add("active");
    const removeActive = link => menu_links[link] && menu_links[link].classList.remove("active");
    ...
    window.addEventListener("scroll", () => {
        ...
        if(current < 4) {
            document.querySelector(".link-sessions").classList.add('active')
        } else {
            document.querySelector(".link-sessions").classList.remove('active')
        }
    });
}, false);
...
function scrollToId(e, id) {
    e.closest("aside").querySelectorAll(".active").forEach(e =>
